...
function showDecisionOverlay(scoredMoves, options = {}) {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    const cubeColor = isFocused || isBest ? 0xffffff : pieceColor;
    const baseOther = Math.max(0.06, otherOpacity - sourceIndex * 0.018);
    const cubeOpacity = isFocused || isBest ? bestOpacity : baseOther;
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  await previewDecisionCycle(topCandidates, signal);

  if (varietyMode === VarietyMode.Deterministic) {
    await confirmDecision(topCandidates, 0, signal);
  ...
  for (let index = 0; index < pool.length; index += 1) {
    ticket -= (topK - index);
    if (ticket <= 0) {
      const chosenIndex = topCandidates.findIndex((entry) => entry.move === pool[index].move);
      await confirmDecision(topCandidates, Math.max(0, chosenIndex), signal);
...
function estimateAIBudgetMs(legalMoveCount) {
  const count = Math.min(DECISION_MAX_CANDIDATES, Math.max(1, legalMoveCount));
  const plan = getDecisionPreviewPlan(count);
  const previewMs = plan.shown * plan.dwellMs + 360;
