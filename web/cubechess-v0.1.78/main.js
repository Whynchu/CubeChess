...
const TRAIL_EMIT_STEP = 0.4;
const TRAIL_POOL_MAX = 140;
const TRAIL_RIBBON_MIN_LEN = 0.05;
const TRAIL_RIBBON_BASE_WIDTH = 0.16;
const TRAIL_RIBBON_BASE_WIDTH_KING = 0.19;
...
function spawnTrailSegment(visual, fromPosition, toPosition) {
  ...
  const ribbonBaseOpacity = isKing ? 0.16 : 0.13;
...
function tickTrails() {
  ...
  for (let i = trails.length - 1; i >= 0; i -= 1) {
    ...
    const width = trail.baseWidth * (0.82 + fade * 0.18);
    const length = trail.length * (0.96 + fade * 0.04);
