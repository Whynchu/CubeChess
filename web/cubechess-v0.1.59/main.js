...
import { classifyBoardPhase, createTurnThreatContext, evaluateHeuristicMove } from "../../Runtime/Core/AI/index.js";
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  const candidatePool = pruneScoredCandidates(scored, {
    limit: AI_CANDIDATE_POOL_LIMIT,
    minPerPiece: AI_CANDIDATE_MIN_PER_PIECE,
  });

  let chosenMove = candidatePool[0].move;
  ...
  const rawChosenPieceMoves = scored
...
function showKingTakenFlash(player) {
  ...
  eventFlashTextEl.textContent = player ? `${player} King taken!` : "King taken!";
...
function updateTurnHud() {
  if (turnMachine.phase === TurnPhase.MatchEnded) {
    setCurrentTurnLabel(`Winner: ${turnMachine.winner ?? "None"}`);
    setTurnLabel(lastMoveHudText ? `Last: ${lastMoveHudText}` : "Last: -");
    return;
  }

  setCurrentTurnLabel(`Turn ${matchState.turnCount + 1} • ${matchState.activePlayer}`);
  setTurnLabel(lastMoveHudText ? `Last: ${lastMoveHudText}` : "Last: -");
}
