...
const MODE_CAMERA_PRESETS = Object.freeze({
  [GameModeId.Chaos8P]: Object.freeze({
    position: Object.freeze([11, 12, 14]),
    target: Object.freeze([0, 0, 0]),
  }),
  [GameModeId.Duel2P]: Object.freeze({
    position: Object.freeze([0, 16.5, 12.5]),
    target: Object.freeze([0, 1.8, 0]),
  }),
});

function applyModeCameraFrame({ immediate = false } = {}) {
  const preset = MODE_CAMERA_PRESETS[currentGameModeId] ?? MODE_CAMERA_PRESETS[GameModeId.Chaos8P];
  ...
  if (immediate) {
    ...
    camera.position.lerp(position, 0.85);
  }
...
function resetMatch({ resume = true } = {}) {
  ...
  rebuildPieceVisuals();
  applyModeCameraFrame({ immediate: true });
