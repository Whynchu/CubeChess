...
const PIECE_MODEL_TARGET_SIZE = Object.freeze({
  [PIECE_TYPES.Pawn]: 0.56,
  [PIECE_TYPES.Rook]: 0.58,
  [PIECE_TYPES.Knight]: 0.62,
  [PIECE_TYPES.Bishop]: 0.62,
  [PIECE_TYPES.Queen]: 0.68,
...
function tintModelInstance(modelRoot, color, baseOpacity) {
  ...
  modelRoot.traverse((node) => {
    ...
    const tinted = source.map((material) => {
      ...
      if ("emissive" in next && next.emissive) {
        next.emissive.copy(tintColor).multiplyScalar(0.26);
...
const PIECE_LABEL = Object.freeze({
  [PIECE_TYPES.King]: "K",
  [PIECE_TYPES.Queen]: "Q",
  [PIECE_TYPES.Rook]: "R",
  [PIECE_TYPES.Bishop]: "B",
  [PIECE_TYPES.Knight]: "Kn",
});
...
function makeLabelSprite(text, darkHex) {
  const size = 128;
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
  const ctx = canvas.getContext("2d");
  const hexStr = "#" + darkHex.toString(16).padStart(6, "0");
  ctx.clearRect(0, 0, size, size);
  ctx.font = `bold ${text.length > 1 ? 52 : 64}px Arial`;
...
function makePieceVisual(piece) {
  ...
  const halo = new THREE.Mesh(
    new THREE.SphereGeometry(isKing ? 0.4 : 0.34, 20, 20),
    new THREE.MeshBasicMaterial({
      color,
      transparent: true,
      opacity: isKing ? 0.42 : 0.32,
  ...
  const { sprite: labelSprite, material: labelMaterial } = makeLabelSprite(label, darkColor);
...
function applyPieceDistanceFade() {
  for (const visual of pieceVisuals.values()) {
    ...
    visual.haloMaterial.opacity = (visual.isKing ? 0.42 : 0.32) * fade;
    visual.sparkMaterial.opacity = 0.85 * fade;
    visual.labelMaterial.opacity = Math.max(0.24, fade);
  }
}
...
function buildPieceCandidatesFromScored(scoredMoves) {
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  const pieceCandidates = buildPieceCandidatesFromScored(topCandidates);
  const chosenPieceIndex = pieceCandidates.findIndex((entry) => entry.pieceId === chosenMove.pieceId);

  await previewPieceSelectionCycle(pieceCandidates, Math.max(0, chosenPieceIndex), signal);
  await confirmPieceSelection(pieceCandidates, Math.max(0, chosenPieceIndex), signal);

  const chosenPieceMoves = topCandidates.filter((entry) => entry.move.pieceId === chosenMove.pieceId);
  const chosenMoveIndex = chosenPieceMoves.findIndex((entry) => entry.move === chosenMove);

  await previewDecisionCycle(chosenPieceMoves, Math.max(0, chosenMoveIndex), signal);
  await confirmDecision(chosenPieceMoves, Math.max(0, chosenMoveIndex), signal);
...
const TRAIL_HALO_GEO = new THREE.SphereGeometry(0.34, 16, 16);
const TRAIL_HALO_KING_GEO = new THREE.SphereGeometry(0.4, 16, 16);
const TRAIL_SPARK_GEO = new THREE.SphereGeometry(0.1, 14, 14);
const TRAIL_SPARK_KING_GEO = new THREE.SphereGeometry(0.12, 14, 14);
...
function spawnTrailGhost(visual, position) {
  ...
  const coreBaseOpacity = 0.2;
  const haloBaseOpacity = isKing ? 0.24 : 0.18;
  const sparkBaseOpacity = 0.12;
  ...
  const core = new THREE.Mesh(isKing ? TRAIL_CORE_KING_GEO : TRAIL_CORE_GEO, coreMaterial);
  const halo = new THREE.Mesh(isKing ? TRAIL_HALO_KING_GEO : TRAIL_HALO_GEO, haloMaterial);
  const spark = new THREE.Mesh(isKing ? TRAIL_SPARK_KING_GEO : TRAIL_SPARK_GEO, sparkMaterial);
  spark.position.set(0.06, 0.06, 0.06);
  ...
  trails.push({
    ...
    haloMaterial,
    sparkMaterial,
    startMs: performance.now(),    durationMs: TRAIL_DURATION_MS,
    baseScale: 1,
    cellBaseOpacity,
    coreBaseOpacity,
    haloBaseOpacity,
...
function tickTrails() {
  ...
  for (let i = trails.length - 1; i >= 0; i -= 1) {
    ...
    trail.haloMaterial.opacity = trail.haloBaseOpacity * fade;
    trail.sparkMaterial.opacity = trail.sparkBaseOpacity * fade;
    ...
    if (t >= 1) {
      ...
      trail.haloMaterial.dispose();
      trail.sparkMaterial.dispose();
      trails.splice(i, 1);
    }
  }
}
...
function tickAnimations() {
  ...
  for (let i = animations.length - 1; i >= 0; i -= 1) {
    ...
    if (anim.type === "capture") {
      ...
      anim.visual.labelMaterial.opacity = Math.max(0, 1 - t);
...
function buildPreviewIndices(count, shown, targetIndex) {
  if (count <= 0) {
    return [];
  }

  const normalizedTarget = Math.max(0, Math.min(count - 1, targetIndex));
  const cappedShown = Math.max(1, Math.min(count, shown));
  const indices = [];

  for (let i = 0; i < cappedShown - 1; i += 1) {
    indices.push(i);
  }

  if (!indices.includes(normalizedTarget)) {
    indices.push(normalizedTarget);
  }
...
function getPiecePreviewPlan(pieceCount) {
  ...
  let shown = count;
  let dwellMs = Math.max(120, Math.round(200 / clampedSpeed));

  if (clampedSpeed <= 0.5) {
    dwellMs = 340;
    shown = Math.min(count, 6);
  } else if (clampedSpeed <= 1) {
    dwellMs = 220;
    shown = Math.min(count, 5);
  } else {
    dwellMs = 120;
    shown = Math.min(count, 3);
  }

  return { shown, dwellMs };
}
...
async function previewPieceSelectionCycle(pieceCandidates, targetIndex, signal) {
  ...
  const order = buildPreviewIndices(pieceCandidates.length, plan.shown, targetIndex);
  ...
  await delayWithSignal(220, signal);
...
function getDecisionPreviewPlan(candidateCount) {
  ...
  let shown = count;
  let dwellMs = Math.max(120, Math.round(260 / clampedSpeed));

  if (clampedSpeed <= 0.5) {
    dwellMs = 500;
    shown = Math.min(count, 10);
  } else if (clampedSpeed <= 1) {
    dwellMs = 350;
    shown = Math.min(count, 8);
  } else {
    dwellMs = 180;
    shown = Math.min(count, 4);
  }

  return { shown, dwellMs };
}

async function previewDecisionCycle(candidates, targetIndex, signal) {
  ...
  const order = buildPreviewIndices(candidates.length, plan.shown, targetIndex);
  ...
  await delayWithSignal(260, signal);
...
function estimateAIBudgetMs(legalMoveCount) {
  ...
  const pieceCountGuess = Math.max(1, Math.min(10, Math.round(Math.sqrt(moveCount * 1.5))));
  const piecePlan = getPiecePreviewPlan(pieceCountGuess);
  const movePlan = getDecisionPreviewPlan(moveCount);
  const previewMs = piecePlan.shown * piecePlan.dwellMs + movePlan.shown * movePlan.dwellMs + 900;
  return Math.min(AI_BUDGET_MAX_MS, Math.max(AI_BUDGET_MS, previewMs + 200));
}
