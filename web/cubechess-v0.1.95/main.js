...
function makePieceVisual(piece) {
  ...
  if (modelTemplate) {
    ...
    // Keep glow perfectly registered with the core model; only scale outward slightly.
    glowInstance.position.copy(modelInstance.position);
    glowInstance.quaternion.copy(modelInstance.quaternion);
    glowInstance.scale.copy(modelInstance.scale).multiplyScalar(1.1);
