...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  const orderedFromBest = [];
  if (orderedMoves.length > 0) {
    for (let i = 0; i < orderedMoves.length; i += 1) {
      orderedFromBest.push(orderedMoves[(bestIndexInOrdered + i) % orderedMoves.length]);
    }
  }

  const chosenMoveIndex = Math.max(0, orderedFromBest.findIndex((entry) => entry.move === chosenMove));

  await previewDecisionCycle(orderedFromBest, chosenMoveIndex, signal);
  await confirmDecision(orderedFromBest, chosenMoveIndex, signal);
