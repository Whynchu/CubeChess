...
function computeHeuristicDecisionLocal({ legalMoves, player, aiBudgetMs, signal }) {
...
const TRAIL_RIBBON_GEO = new THREE.BoxGeometry(1, 1, 1);
const TRAIL_CORE_GEO = new THREE.SphereGeometry(0.14, 14, 14);
const TRAIL_MAX_ACTIVE = 180;
const TRAIL_MAX_STEPS_PER_TICK = 2;
const TRAIL_EMIT_STEP = 0.16;
const TRAIL_POOL_MAX = 360;
const TRAIL_RIBBON_MIN_LEN = 0.05;
const TRAIL_RIBBON_BASE_WIDTH = 0.22;
const TRAIL_RIBBON_BASE_WIDTH_KING = 0.26;
const trailGhostPool = [];
let trailGhostPoolCreated = 0;

const TRAIL_SEGMENT_START_TMP = new THREE.Vector3();
const TRAIL_SEGMENT_END_TMP = new THREE.Vector3();
const TRAIL_SEGMENT_MID_TMP = new THREE.Vector3();
const TRAIL_SEGMENT_DIR_TMP = new THREE.Vector3();
const TRAIL_SEGMENT_PREV_TMP = new THREE.Vector3();
const TRAIL_SEGMENT_AXIS_Z = new THREE.Vector3(0, 0, 1);
...
function createPooledTrailGhost() {
  ...
  const root = new THREE.Group();
  const ribbon = new THREE.Mesh(TRAIL_RIBBON_GEO, ribbonMaterial);
...
function releasePooledTrailGhost(ghost) {
  ...
  ghost.root.scale.set(1, 1, 1);
  ghost.root.quaternion.identity();
  trailGhostPool.push(ghost);
}

function spawnTrailSegment(visual, fromPosition, toPosition) {
  if (trails.length >= TRAIL_MAX_ACTIVE) {
    return;
  }

  TRAIL_SEGMENT_START_TMP.copy(fromPosition);
  TRAIL_SEGMENT_END_TMP.copy(toPosition);
  const length = TRAIL_SEGMENT_START_TMP.distanceTo(TRAIL_SEGMENT_END_TMP);
  if (length < TRAIL_RIBBON_MIN_LEN) {
    return;
  }
  ...
  const ribbonBaseOpacity = isKing ? 0.2 : 0.16;
  const coreBaseOpacity = isKing ? 0.14 : 0.1;
  const baseWidth = isKing ? TRAIL_RIBBON_BASE_WIDTH_KING : TRAIL_RIBBON_BASE_WIDTH;

  TRAIL_SEGMENT_DIR_TMP.subVectors(TRAIL_SEGMENT_END_TMP, TRAIL_SEGMENT_START_TMP).normalize();
  TRAIL_SEGMENT_MID_TMP.addVectors(TRAIL_SEGMENT_START_TMP, TRAIL_SEGMENT_END_TMP).multiplyScalar(0.5);

  pooledGhost.root.visible = true;
  pooledGhost.root.position.copy(TRAIL_SEGMENT_MID_TMP);
  pooledGhost.root.quaternion.setFromUnitVectors(TRAIL_SEGMENT_AXIS_Z, TRAIL_SEGMENT_DIR_TMP);

  pooledGhost.ribbon.material.color.setHex(color);
  pooledGhost.core.material.color.setHex(color);
  pooledGhost.ribbon.material.opacity = ribbonBaseOpacity;
  pooledGhost.core.material.opacity = coreBaseOpacity;

  pooledGhost.root.scale.set(baseWidth, baseWidth, length);
  pooledGhost.core.position.set(0, 0, length * 0.5);
  pooledGhost.core.scale.setScalar(isKing ? 1.15 : 1);

  pieceLayer.add(pooledGhost.root);
  trails.push({
    root: pooledGhost.root,
    ribbonMaterial: pooledGhost.ribbonMaterial,
...
function tickTrails() {
  ...
  for (let i = trails.length - 1; i >= 0; i -= 1) {
    ...
    trail.ribbonMaterial.opacity = trail.ribbonBaseOpacity * fade;
    trail.coreMaterial.opacity = trail.coreBaseOpacity * fade;

    const width = trail.baseWidth * (0.78 + fade * 0.22);
    const length = trail.length * (0.94 + fade * 0.06);
    trail.root.scale.set(width, width, length);
    ...
    if (anim.type === "move") {
      ...
      if (anim.trailEnabled) {
        ...
        if (travel >= TRAIL_EMIT_STEP) {
          ...
          TRAIL_SEGMENT_PREV_TMP.copy(anim.lastTrailPoint);
          for (let step = 1; step <= steps; step += 1) {
            const alpha = step / steps;
            TRAIL_GHOST_TMP.lerpVectors(anim.lastTrailPoint, current, alpha);
            spawnTrailSegment(anim.visual, TRAIL_SEGMENT_PREV_TMP, TRAIL_GHOST_TMP);
            TRAIL_SEGMENT_PREV_TMP.copy(TRAIL_GHOST_TMP);
          }
      ...
      if (t >= 1) {
        if (anim.trailEnabled) {
          spawnTrailSegment(anim.visual, anim.lastTrailPoint, anim.end);
        }
        animations.splice(i, 1);
      }
      continue;
