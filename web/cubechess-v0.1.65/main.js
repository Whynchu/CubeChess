...
import { applyDangerAwareRescoring, classifyBoardPhase, createTurnThreatContext, evaluateHeuristicMove } from "../../Runtime/Core/AI/index.js";
...
const AI_DANGER_MAX_CANDIDATES = 24;
const AI_DANGER_OPPONENT_REPLY_LIMIT = 40;
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  const dangerRescored = applyDangerAwareRescoring({
    scoredMoves: scored,
    matchState,
    player: context.player ?? matchState.activePlayer,
    maxCandidates: AI_DANGER_MAX_CANDIDATES,
    opponentMoveLimit: AI_DANGER_OPPONENT_REPLY_LIMIT,
    dangerWeight: AI_DANGER_WEIGHT,
    signal,
  });

  dangerRescored.sort((a, b) => {
    if (a.score !== b.score) {
      return b.score - a.score;
    }
    if (a.move.pieceId !== b.move.pieceId) {
