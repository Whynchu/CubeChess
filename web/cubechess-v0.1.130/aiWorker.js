...
import { applyValidatedMove, collectLegalMovesForPlayer, getNextActivePlayer } from "./runtime/Core/Turn/turnStateMachine.js";
...
function reconstructMatchContext(snapshot) {
  ...
  const matchState = new MatchState({
    ...
    resultType: snapshot.resultType ?? null,
    enPassantTarget: snapshot.enPassantTarget ? { ...snapshot.enPassantTarget } : null,
    noProgressHalfmoveClock: snapshot.noProgressHalfmoveClock ?? 0,
...
function cloneMatchContext(matchState) {
  ...
  const clone = new MatchState({
    ...
    noProgressHalfmoveClock: matchState.noProgressHalfmoveClock ?? 0,
    repetitionCounts: matchState.repetitionCounts ?? {},
  });
...
function applyMoveToClone(matchState, occupancyMap, move) {
  try {
    applyValidatedMove(matchState, occupancyMap, move);
    const nextPlayer = getNextActivePlayer(matchState, matchState.activePlayer);
    if (nextPlayer) {
      matchState.activePlayer = nextPlayer;
    }
    if (matchState.gameModeId === "duel_2p") {
      const key = buildDuelRepetitionStateKey(matchState);
      matchState.repetitionCounts = {
        ...(matchState.repetitionCounts ?? {}),
        [key]: (Number(matchState.repetitionCounts?.[key] ?? 0) || 0) + 1,
      };
    }
    return true;
  } catch {
