...
const TRAIL_BASE_SIZE = 0.2;

function pushTrail(color, fromWorld, toWorld, moveDistanceCells) {
  if (moveDistanceCells < TRAIL_MIN_MOVE_CELLS) {
    return;
  }

  const headColor = new THREE.Color(color);
  const tailColor = new THREE.Color(color).lerp(new THREE.Color(0xffffff), 0.45);
  const sprites = [];
  const materials = [];

  for (let i = 0; i < TRAIL_SEGMENTS; i += 1) {
    const t = i / (TRAIL_SEGMENTS - 1);
    const position = new THREE.Vector3().lerpVectors(fromWorld, toWorld, t);
    const segmentColor = new THREE.Color().lerpColors(headColor, tailColor, t);
    const segmentOpacity = 0.16 + t * 0.5;
    const segmentSize = TRAIL_BASE_SIZE * (0.7 + t * 1.1);
    ...
    sprite.scale.setScalar(segmentSize);
    sprite.renderOrder = 14;

    pieceLayer.add(sprite);
    materials.push(material);
    sprites.push({ sprite, baseOpacity: segmentOpacity, baseSize: segmentSize });
  }
...
function tickTrails() {
  ...
  for (let i = trails.length - 1; i >= 0; i -= 1) {
    ...
    for (const segment of trail.sprites) {
      segment.sprite.material.opacity = segment.baseOpacity * fade;
      segment.sprite.scale.setScalar(segment.baseSize * (0.92 + fade * 0.18));
    }

    if (t >= 1) {
      for (const segment of trail.sprites) {
        pieceLayer.remove(segment.sprite);
      }
...
function pushMoveAnimation(pieceId, from, to, durationMs = 260) {
  ...
  pushTrail(visual.color, fromWorld, toWorld, moveDistanceCells);
}
