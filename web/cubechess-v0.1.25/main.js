...
function showDecisionOverlay(scoredMoves) {
  clearDecisionOverlay();
  if (!Array.isArray(scoredMoves) || scoredMoves.length === 0) {
    return;
  }

  const candidates = scoredMoves.slice(0, DECISION_MAX_CANDIDATES);
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    const cubeColor = isBest ? 0xffffff : pieceColor;
    const cubeOpacity = isBest ? 0.34 : Math.max(0.08, 0.22 - index * 0.018);
