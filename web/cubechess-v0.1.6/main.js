...
function createStartingPieces() {
  ...
  const voxelGeo = new THREE.BoxGeometry(0.78, 0.78, 0.78);
  const kingCoreGeo = new THREE.SphereGeometry(0.28, 24, 24);
  const coreGeo = new THREE.SphereGeometry(0.24, 22, 22);
  const kingHaloGeo = new THREE.SphereGeometry(0.4, 20, 20);
  const haloGeo = new THREE.SphereGeometry(0.34, 18, 18);
  ...
  for (const key of Object.keys(corners)) {
    ...
    for (let i = 0; i < layout.length; i += 1) {
      ...
      voxel.renderOrder = 10;

      const voxelEdges = new THREE.LineSegments(
        new THREE.EdgesGeometry(voxelGeo),
      ...
      voxel.add(voxelEdges);

      const core = new THREE.Mesh(
        i === 0 ? kingCoreGeo : coreGeo,
        new THREE.MeshBasicMaterial({
          color: corner.color,
          opacity: 1,
          depthTest: false,
