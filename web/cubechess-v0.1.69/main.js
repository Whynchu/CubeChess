...
function makePieceVisual(piece) {
  ...
  return {
    ...
    bobRate: 0.55 + bobSeed * 0.45,
    bobAmplitude: isKing ? 0.07 : 0.055,
  };
}
...
const animations = [];
const trails = [];
const TRAIL_DURATION_MS = 340;
const TRAIL_MIN_MOVE_CELLS = 1;
const TRAIL_EMIT_STEP = 0.28;
...
const TRAIL_MAX_ACTIVE = 120;
const TRAIL_MAX_STEPS_PER_TICK = 2;
const TRAIL_GHOST_TMP = new THREE.Vector3();

function spawnTrailGhost(visual, position, options = {}) {
  ...
  if (includeModel && visual.modelTemplate) {
...
function pushMoveAnimation(pieceId, from, to, durationMs = 260) {
  ...
  if (moveDistanceCells >= TRAIL_MIN_MOVE_CELLS) {
    spawnTrailGhost(visual, fromWorld, { includeModel: true });
  }
}
...
function tickAnimations() {
  ...
  for (let i = animations.length - 1; i >= 0; i -= 1) {
    ...
    if (anim.type === "move") {
      ...
      if (t >= 1) {
        if (anim.trailEnabled) {
          spawnTrailGhost(anim.visual, anim.end, { includeModel: true });
        }
        animations.splice(i, 1);
      }
      continue;
