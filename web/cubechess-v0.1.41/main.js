...
const MODEL_LIGHT_BASE_INTENSITY = 0.42;
const MODEL_LIGHT_KING_BASE_INTENSITY = 0.56;
...
function makePieceVisual(piece) {
  ...
  const lightBaseIntensity = isKing ? MODEL_LIGHT_KING_BASE_INTENSITY : MODEL_LIGHT_BASE_INTENSITY;
  ...
  if (modelTemplate) {
    ...
    pieceLight = new THREE.PointLight(color, lightBaseIntensity, isKing ? 3.2 : 2.8, 2.0);
    ...
    pieceLight = new THREE.PointLight(color, lightBaseIntensity * 0.78, isKing ? 2.9 : 2.5, 2.0);
    root.add(pieceLight);
  }
...
function applyPieceDistanceFade() {
  for (const visual of pieceVisuals.values()) {
    ...
    if (visual.pieceLight) {
      visual.pieceLight.intensity = (visual.lightBaseIntensity ?? 0) * fade;
    }
  }
}
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  if (isSlidingPiece && rawChosenPieceMoves.length > 1 && bestDeterministicForPiece) {
    decisionPreviewSequence = buildSlidingPreviewPath(rawChosenPieceMoves, bestDeterministicForPiece, { move: chosenMove, score: 0 });
  } else {
    ...
    decisionPreviewSequence = [];
    for (let i = 0; i < orderedMoves.length; i += 1) {
      decisionPreviewSequence.push(orderedMoves[(bestIndexInOrdered + i) % orderedMoves.length]);
    }
  }

  const chosenMoveIndex = Math.max(0, decisionPreviewSequence.findIndex((entry) => entry.move === chosenMove));

  await previewDecisionCycle(decisionPreviewSequence, chosenMoveIndex, signal);
  await confirmDecision(decisionPreviewSequence, chosenMoveIndex, signal);
...
function tickAnimations() {
  ...
  for (let i = animations.length - 1; i >= 0; i -= 1) {
    ...
    if (anim.type === "capture") {
      ...
      if (anim.visual.pieceLight) {
        anim.visual.pieceLight.intensity = Math.max(0, (anim.visual.lightBaseIntensity ?? 0) - t * (anim.visual.lightBaseIntensity ?? 0));
      }
...
function buildBranchStepOrder(branches, fromBranch, toBranch, direction) {
  if (branches.length === 0) return [];
  const fromIdx = branches.indexOf(fromBranch);
  const toIdx = branches.indexOf(toBranch);
  if (fromIdx < 0 || toIdx < 0) return [];
  const seq = [];
  let idx = fromIdx;
  while (idx !== toIdx) {
    idx = (idx + direction + branches.length) % branches.length;
    seq.push(branches[idx]);
    if (seq.length > branches.length + 1) break;
  }
  return seq;
}
...
function buildSlidingPreviewPath(entries, startEntry, targetEntry) {
  ...
  const buildPath = (mode) => {
    ...
    const switchBranch = (targetBranch) => {
      ...
      const cw = buildBranchStepOrder(branches, currentBranch, targetBranch, 1);
      const ccw = buildBranchStepOrder(branches, currentBranch, targetBranch, -1);
      const branchSteps = ccw.length < cw.length ? ccw : cw;

      for (const nextBranch of branchSteps) {
        const depths = byBranch.get(nextBranch) ?? [];
        if (depths.length === 0) continue;
        let nextDepth = currentDepth;
        if (!depths.includes(nextDepth)) {
          nextDepth = nearestDepthInBranch(depths, currentDepth);
        }
        const entry = getEntryByBranchDepth(entries, nextBranch, nextDepth);
        if (entry) {
...
async function previewDecisionCycle(candidates, targetIndex, signal) {
  ...
  const order = buildCyclePathIndices(candidates.length, 0, targetIndex);
