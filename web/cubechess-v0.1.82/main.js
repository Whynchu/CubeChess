...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  if (usedChaoticRerank) {
    ...
    scoreGapTop2 = computeScoreGapTop2(selectionPool);
  }

  let chosenMove = selectionPool[0]?.move ?? dangerRescored[0]?.move ?? scored[0]?.move ?? legalMoves[0] ?? null;
  if (!chosenMove) {
    clearDecisionOverlay();
    return null;
  }

  if (varietyMode !== VarietyMode.Deterministic && selectionPool.length > 0) {
    const topK = getChaoticTopKForGap(scoreGapTop2, selectionPool.length);
    samplingTopK = topK;
    const pool = selectionPool.slice(0, topK);
  ...
  const topCandidates = selectionPool.slice(0, DECISION_MAX_CANDIDATES);
