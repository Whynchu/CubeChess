...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  await previewPieceSelectionCycle(pieceCandidates, signal);
  ...
  await previewDecisionCycle(chosenPieceMoves, signal);
...
async function previewPieceSelectionCycle(pieceCandidates, signal) {
  if (!Array.isArray(pieceCandidates) || pieceCandidates.length === 0) {
    clearDecisionOverlay();
    return;
  }
  ...
  for (let index = 0; index < plan.shown; index += 1) {
    if (signal?.aborted) {
      break;
    }
    showPieceSelectionOverlay(pieceCandidates, {
...
async function previewDecisionCycle(candidates, signal) {
  ...
  // First, show the full candidate field so the tree is obvious.
