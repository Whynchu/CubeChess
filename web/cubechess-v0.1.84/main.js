...
function exportAIDecisionTraces() {
  ...
  const payload = {
    ...
    kpiSummary: buildTraceKpiSummary(aiDecisionTraces),
...
const CHAOTIC_TACTICAL_SCORE_DROP = 2.6;
const CHAOTIC_TACTICAL_DANGER_MARGIN = 2.2;
const CHAOTIC_TACTICAL_MIN_POOL = 2;
...
function buildTacticalSelectionPool(selectionPool) {
  ...
  const filtered = selectionPool.filter((entry, index) => {
    ...
    return score >= bestScore - CHAOTIC_TACTICAL_SCORE_DROP
      && danger <= bestDanger + CHAOTIC_TACTICAL_DANGER_MARGIN;
  });

  if (filtered.length >= Math.min(CHAOTIC_TACTICAL_MIN_POOL, selectionPool.length)) {
    return filtered;
  }

  return selectionPool.slice(0, Math.min(CHAOTIC_TACTICAL_MIN_POOL, selectionPool.length));
}
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  let selectionPool = candidatePool;
  let tacticalPool = candidatePool;
  let selectedBy = "deterministic_best";
  let scoreGapTop2 = computeScoreGapTop2(candidatePool);
  let samplingTopK = 1;
  let tacticalFilterActive = false;
  let tacticalRejectedCount = 0;
  const usedChaoticRerank = varietyMode !== VarietyMode.Deterministic && candidatePool.length > 0;

  if (usedChaoticRerank) {
    selectionPool = buildChaoticSelectionPool(candidatePool, aiRecentMoves, player);
    tacticalPool = buildTacticalSelectionPool(selectionPool);
