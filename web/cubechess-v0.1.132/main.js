...
function updateModeDescription() {
  ...
  modeDescriptionEl.textContent = currentGameModeId === GameModeId.Duel2P
    ? 'Two-player wall duel with full orthodox chess formation.'
    : '8-player voxel strategy tuned for fast AI spectacle.';
}
...
const AI_PERSONA_TUNING = Object.freeze({
  ...
  red_aggressor: Object.freeze({
    ...
    candidateConfig: Object.freeze({ poolLimit: 106, minPerPiece: 2 }),
    searchConfig: Object.freeze({
      budgetFraction: 0.39,
    ...
    riskGate: Object.freeze({ maxCombinedRisk: 8.6, maxCounterRisk: 5.0 }),
  }),
  orange_raider: Object.freeze({
    ...
    riskGate: Object.freeze({ maxCombinedRisk: 7.2, maxCounterRisk: 4.3 }),
  }),
  yellow_opportunist: Object.freeze({
    ...
    candidateConfig: Object.freeze({ poolLimit: 98, minPerPiece: 2 }),
    searchConfig: Object.freeze({
      budgetFraction: 0.38,
    ...
    riskGate: Object.freeze({ maxCombinedRisk: 6.8, maxCounterRisk: 4.0 }),
  }),
  green_swarm: Object.freeze({
    ...
    candidateConfig: Object.freeze({ poolLimit: 114, minPerPiece: 3 }),
    searchConfig: Object.freeze({
      budgetFraction: 0.4,
      rootCandidateLimit: 17,
      depth3CandidateLimit: 8,
      opponentMoveLimit: 12,
      selfMoveLimit: 14,
      replyWeight: 0.9,
  ...
  blue_fortress: Object.freeze({
    ...
    riskGate: Object.freeze({ maxCombinedRisk: 5.2, maxCounterRisk: 2.9 }),
  }),
...
let currentDuelPlayers = ["Yellow", "Red"];
...
function getCurrentActivePlayers() {
  return currentGameModeId === GameModeId.Duel2P ? [...currentDuelPlayers] : [...(getGameModeDefinition(currentGameModeId).activePlayers ?? TURN_ORDER)];
}
...
function getCurrentDuelMatchup() {
  ...
  const duelPlayers = getCurrentActivePlayers();
...
function updatePlayerStatsPanel() {
  ...
  const activePlayers = getCurrentActivePlayers();
...
function sampleDuelColorPair() {
  const pool = [...TURN_ORDER];
  ...
  return [pool[0] ?? "Yellow", pool[1] ?? "Red"];
}
...
function refreshMatchPersonaAssignments() {
  ...
  if (currentGameModeId === GameModeId.Duel2P) {
    currentDuelPlayers = sampleDuelColorPair();
    for (const player of currentDuelPlayers) {
...
function buildMatchSnapshotForAIWorker(state) {
  return {
    ...
    gameModeId: state.gameModeId ?? GameModeId.Chaos8P,
    resultType: state.resultType ?? null,
    enPassantTarget: state.enPassantTarget ? { ...state.enPassantTarget } : null,
    noProgressHalfmoveClock: Number(state.noProgressHalfmoveClock ?? 0) || 0,
    ...
    pieces: state.pieces.map((piece) => ({
      ...
      type: piece.type,
      alive: piece.alive,
      forward: piece.forward ? { ...piece.forward } : null,
      hasMoved: piece.hasMoved === true,
      coord: {
        x: piece.coord.x,
        y: piece.coord.y,
        z: piece.coord.z,
...
function nextDecisionOverlayEpoch() {
  decisionOverlayEpoch += 1;
  return decisionOverlayEpoch;
}

function isDecisionOverlayEpochCurrent(epoch) {
  return epoch === decisionOverlayEpoch;
}
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  const overlayEpoch = decisionOverlayEpoch;
  ...
  if (pieceCandidatesAtStart.length > 0 && isDecisionOverlayEpochCurrent(overlayEpoch)) {
    ...
    if (!isDecisionOverlayEpochCurrent(overlayEpoch)) {
      clearDecisionOverlay();
  ...
  await previewPieceSelectionCycle(pieceCandidates, chosenPieceIndex, signal, overlayEpoch);
  await confirmPieceSelection(pieceCandidates, chosenPieceIndex, signal, overlayEpoch);
  if (!isDecisionOverlayEpochCurrent(overlayEpoch)) {
    clearDecisionOverlay();
    return chosenMove;
  }
  ...
  await previewDecisionCycle(decisionCandidates, chosenMoveIndex, signal, focusOrder, overlayEpoch);
  await confirmDecision(decisionCandidates, chosenMoveIndex, signal, overlayEpoch);
  if (!isDecisionOverlayEpochCurrent(overlayEpoch)) {
    clearDecisionOverlay();
    return chosenMove;
  }
...
function setActiveTurnTint(player) {
  ...
  root.style.setProperty("--turn-tint", hexToRgbaCss(PLAYER_COLOR[player], 0.02));
}
...
function resetMatch({ resume = true } = {}) {
  ...
  const initial = initializeMatchState({ gameModeId: currentGameModeId, seatOffset: currentSeatOffset, activePlayers: getCurrentActivePlayers() });
...
function enableTouchScrollShim(container) {
  if (!container || !("ontouchstart" in window) && !(navigator.maxTouchPoints > 0)) {
    return;
  }
  ...
  const handleTouchMove = (event) => {
    ...
    const touch = [...event.touches].find((entry) => entry.identifier === activeTouchId) ?? event.touches[0];
    if (!touch) {
      return;
    }
  ...
  container.addEventListener("touchstart", (event) => {
    ...
    active = container.scrollHeight > container.clientHeight;
  ...
  window.addEventListener("touchend", stopTouchScroll, { passive: true, capture: true });
  window.addEventListener("touchcancel", stopTouchScroll, { passive: true, capture: true });
}
...
async function previewPieceSelectionCycle(pieceCandidates, targetIndex, signal, overlayEpoch = decisionOverlayEpoch) {
  if (!Array.isArray(pieceCandidates) || pieceCandidates.length === 0) {
    clearDecisionOverlay();
    return;
  }
  ...
  for (const index of order) {
    if (signal?.aborted || !isDecisionOverlayEpochCurrent(overlayEpoch)) {
      break;
    }
...
async function confirmPieceSelection(pieceCandidates, chosenIndex, signal, overlayEpoch = decisionOverlayEpoch) {
  if (!Array.isArray(pieceCandidates) || pieceCandidates.length === 0) {
    return;
  }
...
async function previewDecisionCycle(candidates, targetIndex, signal, focusOrder = null, overlayEpoch = decisionOverlayEpoch) {
...
async function confirmDecision(candidates, chosenIndex, signal, overlayEpoch = decisionOverlayEpoch) {
  if (!Array.isArray(candidates) || candidates.length === 0) {
    return;
  }
...
async function runOneTurn() {
  ...
  try {
    ...
    if (begin.type === TurnPhase.AwaitingAIMove) {
      setStatus(`${getPlayerDisplayName(begin.player)} evaluating...`);
...
function applyHudCollapsed(collapsed) {
  ...
  hudToggleBtn.textContent = collapsed ? "Open" : "Close";
...
hudEl?.addEventListener("touchstart", (event) => {
  event.stopPropagation();
}, { passive: true });
hudEl?.addEventListener("touchmove", (event) => {
  event.stopPropagation();
}, { passive: true });
enableTouchScrollShim(hudBodyEl);
