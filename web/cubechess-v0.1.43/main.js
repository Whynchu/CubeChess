...
function makePieceVisual(piece) {
  ...
  if (modelTemplate) {
    ...
    if (experimentalPieceLightEnabled) {
      pieceLight = new THREE.PointLight(color, isKing ? 0.5 : 0.38, isKing ? 3.2 : 2.8, 2.0);
...
function applyPieceDistanceFade() {
  for (const visual of pieceVisuals.values()) {
    ...
    if (visual.pieceLight) {
      const lightBase = visual.isKing ? 0.5 : 0.38;
      visual.pieceLight.intensity = lightBase * fade;
    }
  }
}
...
async function chooseHeuristicAIMove({ legalMoves, signal, ...context }) {
  ...
  if (experimentalBranchDepthEnabled && isSlidingPiece && rawChosenPieceMoves.length > 1 && bestDeterministicForPiece) {
    const chosenEntry = rawChosenPieceMoves.find((entry) => entry.move === chosenMove) ?? { move: chosenMove, score: 0 };
    decisionPreviewSequence = buildSlidingPreviewPath(rawChosenPieceMoves, bestDeterministicForPiece, chosenEntry);
  } else {
  ...
  for (let i = animations.length - 1; i >= 0; i -= 1) {
    ...
    if (anim.type === "capture") {
      ...
      if (anim.visual.pieceLight) {
        const lightBase = anim.visual.isKing ? 0.5 : 0.38;
        anim.visual.pieceLight.intensity = Math.max(0, lightBase - t * lightBase);
      }
