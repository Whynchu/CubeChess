...
function pushTrail(color, fromWorld, toWorld) {
  const points = [fromWorld.clone(), toWorld.clone()];
  const geo = new THREE.BufferGeometry().setFromPoints(points);
  ...
  line.renderOrder = 7;
  pieceLayer.add(line);
  trails.push({ mesh: line, mat, geo, startMs: performance.now(), durationMs: 1000 });
}
...
function tickTrails() {
  ...
  for (let i = trails.length - 1; i >= 0; i -= 1) {
    ...
    trail.mat.opacity = 0.10 * (1 - t);
    if (t >= 1) {
      pieceLayer.remove(trail.mesh);
      trail.geo.dispose();
      trail.mat.dispose();
      trails.splice(i, 1);
    }
...
function pushMoveAnimation(pieceId, from, to, durationMs = 260) {
  ...
  pushTrail(visual.color, fromWorld, toWorld);
}
